...
const Documents = () => {
  ...
  const deleteDoc = (e) => {
    docsDelete(e).then((Response) => console.log(Response.status));
  };

  const uploadDoc = () => {
    if (uId && updateVal == "update") {
      docsUpdate(uId, body).then((json) => console.log(json));
      setUId("");
    } else if (updateVal == "submit") {
      docsPost(body).then((json) => console.log(json));
      setUId("");
    }
  };
  ...
  return (
    <div style={{ marginLeft: "2px" }}>
      ...
      <div style={{ marginTop: "4rem", marginBottom: "5rem" }}>
        <ClayButton displayType="primary" onClick={getDocs}>
          Refresh Docs
        </ClayButton>
      </div>
      ...
      {items.map((doc) => {
        if (
          doc.encodingFormat == "image/png" ||
          doc.encodingFormat == "image/jpg" ||
          doc.encodingFormat == "image/jpeg"
        ) {
          return (
            <div className="docshitesh" key={doc.id}>
              <h6>ID: {doc.id}</h6>

              <div style={{ position: "relative" }}>
                <img
                  className="docimghitesh"
                  src={doc.adaptedImages[1].contentUrl}
                  alt=""
                />
              </div>
