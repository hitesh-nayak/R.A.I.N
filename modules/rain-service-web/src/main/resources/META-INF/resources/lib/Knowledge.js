...
const Knowledge = () => {
  ...
  const getArticles = () => {
    knowledgeGet().then(function (json) {
      data = json.items;
      setItems(data);
    });
  };
  ...
  const deleteArticle = () => {
    if (id) {
      knowledgeDelete(id).then((Response) => console.log(Response.status));
  ...
  const createArticle = () => {
    if (uId && updateVal == "update") {
      knowledgeUpdate(uId, JSON.stringify(article)).then((json) =>
        console.log(json)
      );
      setUId("");
      setArticle(newArticle);
    } else if (updateVal == "submit") {
      knowledgePost(JSON.stringify(article)).then((json) => console.log(json));
      setUId("");
      getArticles();
      setArticle(newArticle);
    }
  ...
  return (
    <div style={{ marginLeft: "2px" }}>
      ...
      <div style={{ marginTop: "4rem", marginBottom: "5rem" }}>
        <ClayButton displayType="primary" onClick={getArticles}>
          Refresh Articles
        </ClayButton>
      </div>
